...
type RedPacket struct {
	...
	RedPacketType  string         `gorm:"column:red_packet_type;type:varchar(20);not null;default:'normal';index;comment:分配类型(normal普通红包,lucky拼手气红包)"`
	...
	Amount         int64          `gorm:"column:amount;type:bigint;not null;comment:红包金额(单位:分,群聊时为单个红包金额)"`
	TotalAmount    int64          `gorm:"column:total_amount;type:bigint;not null;comment:总金额(单位:分,群聊时为总金额,私聊时等于amount)"`
	Count          int32          `gorm:"column:count;type:int;not null;default:1;comment:红包个数(群聊时使用,私聊时为1)"`
	RemainingAmount int64         `gorm:"column:remaining_amount;type:bigint;not null;comment:剩余金额(单位:分,用于并发控制)"`
	RemainingCount  int32         `gorm:"column:remaining_count;type:int;not null;default:1;comment:剩余个数(用于并发控制)"`
...
type RedPacketReceive struct {
	...
	RedPacketID int64     `gorm:"column:red_packet_id;not null;index;comment:红包ID"`
	UserID     int64     `gorm:"column:user_id;not null;index;comment:领取者ID"`
